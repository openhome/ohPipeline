...
Msg* SuiteDecodedAudioAggregator::ProcessMsg(MsgStreamSegment* /*aMsg*/)
{
    ASSERTS();
    return nullptr;
}
...
void SuiteDecodedAudioAggregator::TestDsdAggregated()
{
    ...
    for (TUint i = 0; i < kNumDsdMsg; i++) {
        auto audio = iMsgFactory->CreateMsgAudioDsd(decodedAudioBuf, 2, kSampleRateDsd, 2, iTrackOffset, 0);
        Queue(audio);
