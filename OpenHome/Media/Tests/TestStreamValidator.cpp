...
void SuiteStreamValidator::MsgsPassThrough()
{
    EMsgType types[] = { EMsgMode, EMsgTrack, EMsgDrain, EMsgEncodedStream, EMsgDelay,
                         ...
                         EMsgAudioPcm, EMsgAudioDsd, EMsgSilence, EMsgQuit };
    const size_t numElems = sizeof(types) / sizeof(types[0]);
    for (size_t i=0; i<numElems; i++) {
        PushMsg(types[i]);
...
void SuiteStreamValidator::PipelineAnimatorDsdBlockConfiguration(TUint& aSampleBlockWords, TUint& aPadBytesPerChunk) const
{
    aSampleBlockWords = 1;
    aPadBytesPerChunk = 0;
}
